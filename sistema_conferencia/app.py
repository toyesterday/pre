...
def visualizar_conferencia(id_conferencia):
    ...
    if conferencia.id_usuario_responsavel != session["user_id"]:
        ...
        return redirect(url_for("index"))
    return render_template("conferencia_detalhe.html", conferencia=conferencia)
...
def gerar_pdf_conferencia(id_conferencia):
    ...
    conferencia = Conferencia.query.get_or_404(id_conferencia)
    if conferencia.id_usuario_responsavel != session["user_id"]:
        flash("Você não tem permissão para gerar PDF desta conferência.", "danger")
        return redirect(url_for("index"))
    
    html_string = render_template("relatorio_conferencia_pdf.html", conferencia=conferencia, now=datetime.datetime.now())
    pdf_bytes = HTML(string=html_string).write_pdf()

    response = make_response(pdf_bytes)
    response.headers["Content-Type"] = "application/pdf"
    response.headers["Content-Disposition"] = f"attachment; filename=conferencia_os_{conferencia.numero_os}.pdf"
