...
# Usar caminho absoluto para o banco de dados para garantir permissões corretas
...
class Conferencia(db.Model):
    ...
    envio_faca_cliche = db.relationship("EnvioFacaCliche", backref="conferencia_pai", lazy=True, uselist=False, cascade="all, delete-orphan")
...
# Novo modelo para controle de envio de faca/clichê/fotolito
class EnvioFacaCliche(db.Model):
    __tablename__ = "EnviosFacaCliche"
    id_envio = db.Column(db.Integer, primary_key=True, autoincrement=True)
    id_conferencia = db.Column(db.Integer, db.ForeignKey("Conferencias.id_conferencia"), nullable=False)
    id_usuario_responsavel = db.Column(db.Integer, db.ForeignKey("Usuarios.id_usuario"), nullable=False)
    status_envio = db.Column(db.String(50), default="Pendente")  # Pendente, Enviado
    data_envio = db.Column(db.DateTime, default=datetime.datetime.utcnow)
    observacoes_envio = db.Column(db.Text)
    responsavel_envio = db.relationship("Usuario", foreign_keys=[id_usuario_responsavel])
...
@app.route("/conferencia/<int:id_conferencia>/atualizar-status-envio", methods=["POST"])
def atualizar_status_envio(id_conferencia):
    if "user_id" not in session:
        return redirect(url_for("login"))
    
    try:
        # Buscar a conferência
        conferencia = Conferencia.query.get(id_conferencia)
        
        # Verificar se a conferência existe
        if not conferencia:
            flash(f"A conferência com ID {id_conferencia} não foi encontrada.", "warning")
            return redirect(url_for("index"))
        
        # Verificar permissão (qualquer usuário pode atualizar o status de envio)
        if conferencia.id_usuario_responsavel != session["user_id"]:
            flash("Você não tem permissão para atualizar esta conferência.", "danger")
            return redirect(url_for("index"))
        
        # Obter dados do formulário
        status_envio = request.form.get("status_envio")
        observacoes_envio = request.form.get("observacoes_envio")
        
        # Verificar se já existe um registro de envio para esta conferência
        envio_existente = EnvioFacaCliche.query.filter_by(id_conferencia=id_conferencia).first()
        
        if envio_existente:
            # Atualizar o registro existente
            envio_existente.status_envio = status_envio
            envio_existente.observacoes_envio = observacoes_envio
            envio_existente.data_envio = datetime.datetime.utcnow()
            envio_existente.id_usuario_responsavel = session["user_id"]
        else:
            # Criar um novo registro de envio
            novo_envio = EnvioFacaCliche(
        ...
        flash(f"Status de envio atualizado com sucesso para: {status_envio}", "success")
        return redirect(url_for("visualizar_conferencia", id_conferencia=id_conferencia))
        
    except Exception as e:
        logger.error(f"Erro ao atualizar status de envio da conferência {id_conferencia}: {str(e)}")
        db.session.rollback()
        flash(f"Erro ao atualizar status de envio: {str(e)}", "danger")
        return redirect(url_for("visualizar_conferencia", id_conferencia=id_conferencia))
